...
bool Cell_Update(bool *Subset, bool *aux_grid, /*vector<bool> &Grid*/ int X, int Y)
{
    vector<bool> Grid (aux_grid, aux_grid + (Order*Order));
    copy(aux_grid, aux_grid + (Order*Order), Grid.begin());
...
{
	int X, Y;
    bool aux_grid[Order*Order];
    
    copy(Grid.begin(), Grid.end(), aux_grid);

    //bool Subset[Elem_Per_Proc];
    //bool New_Subset[Elem_Per_Proc]; 
    
    bool *Subset = new bool[Elem_Per_Proc];
    bool *New_Subset = new bool[Elem_Per_Proc]; 

    MPI_Scatter(aux_grid, Elem_Per_Proc, MPI_CXX_BOOL, Subset, Elem_Per_Proc, MPI_CXX_BOOL, 0, MPI_COMM_WORLD);
    
	for (X = 0; X < Elem_Per_Proc/Order; X++)
		for (Y = 0; Y < Elem_Per_Proc/Order; Y++){
            //New_Grid[X * Order + Y] = Cell_Update(Grid, X, Y);
            New_Subset[X * Order + Y] = Cell_Update(Subset, aux_grid, X, Y);
        }
    
    //bool *aux_new_grids = new bool[Order*Order];   
    bool aux_new_grids[Order*Order];    

    MPI_Barrier(MPI_COMM_WORLD);
    
    MPI_Allgather(New_Subset, Elem_Per_Proc, MPI_CXX_BOOL, &aux_new_grids, Order*Order, MPI_CXX_BOOL, MPI_COMM_WORLD);
    /*if(Proc_Rank == 0){
        for (X = 0; X < Elem_Per_Proc/Order; X++)
            for (Y = 0; Y < Order; Y++){
                New_Grid[X * Order + Y] = Cell_Update(Subset, aux_grid, X, Y);
            }
    }*/
    
    copy(aux_new_grids, aux_new_grids + (Order*Order), New_Grid.begin());
    delete[] Subset, New_Subset;
}
