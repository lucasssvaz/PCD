...
int Cells_Total(vector<vector<bool>> &Grid)
{
	...
	#pragma omp parallel for private(i, j) shared(Cells_Partial)
		for (i = 0; i < Grid.size(); i++)
			for (j = 0; j < Grid[i].size(); j++)
				if (Grid[i][j] == 1)
					Cells_Partial[omp_get_thread_num()]++;

	for (i = 0; i < N_THREADS; i++)
	{
		//cout << endl << Cells_Partial[i];
		Cells_Sum += Cells_Partial[i];
	}
